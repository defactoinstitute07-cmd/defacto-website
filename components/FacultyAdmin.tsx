...
export default function FacultyAdmin() {
  ...
  return (
    <section className="max-w-4xl mx-auto py-8 text-slate-100">
        ...
        <form onSubmit={handleAddFaculty} className="space-y-8">
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-6">
              <label className="cursor-pointer bg-white text-slate-900 px-8 py-3 rounded-xl font-black text-xs shadow-xl hover:bg-slate-200 transition-all uppercase tracking-wider">
              ...
              {imageUrl && (
                <div className="flex items-center gap-4 animate-in fade-in slide-in-from-left-4 duration-300">
                  <img
                    src={imageUrl}
                    className="h-16 w-16 rounded-2xl object-cover ring-2 ring-amber-500/50"
                    alt="Faculty preview"
                  />
                  <span className="text-xs font-bold text-emerald-400">Photo Ready</span>
                </div>
              )}
            </div>
          </div>

          {status && <div className="text-sm font-bold text-amber-400 drop-shadow-sm">{status}</div>}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {faculty.map((f) => (
          <div
            key={f.id}
            className="bg-slate-900 border border-white/5 rounded-[2rem] p-6 shadow-xl relative group overflow-hidden transition-all hover:border-white/10"
          >
            <div className="flex flex-col items-center text-center relative z-10">
              <div className="relative mb-4">
                <img
                  src={f.imageUrl}
                  alt={f.name}
                  className="h-24 w-24 object-cover rounded-3xl ring-4 ring-slate-800 transition-transform duration-500 group-hover:scale-110"
                />
                <div className="absolute -bottom-2 -right-2 bg-amber-400 p-1.5 rounded-lg shadow-lg">
              ...
              <h3 className="font-bold text-white text-lg tracking-tight mb-1">{f.name}</h3>
              <p className="text-slate-500 text-xs font-bold uppercase tracking-widest">
                {f.designation}
              </p>

              <button
                className="mt-6 text-[10px] font-black tracking-widest text-slate-600 hover:text-rose-500 transition-colors uppercase"

  ...
  return (
    <>
      <header className="fixed inset-x-0 top-0 z-[120] border-b border-white/10 bg-slate-950/92 pt-[env(safe-area-inset-top)] font-sans backdrop-blur-xl supports-[backdrop-filter]:bg-slate-950/80">
        <div className="mx-auto flex h-[72px] max-w-[1500px] items-center justify-between gap-3 px-5 sm:h-[78px] sm:px-6 lg:h-[84px] lg:px-8">
          <div className="flex flex-col">
            <Link
              href="/"
              className="inline-flex items-center text-lg font-bold tracking-tight transition hover:opacity-80"
            >
              <BrandLogo variant="light" className="h-8 sm:h-9 lg:h-10" />
            </Link>
          </div>
          ...
          <div className="hidden lg:flex items-center gap-8">
            <nav
              aria-label="Primary"
              className="flex items-center gap-6 text-sm font-semibold text-slate-300"
            >
              {navigation.map((item) => (
                <Link
                  key={item.href}
                  ...
                  className={`transition-colors hover:text-amber-400 ${
                    isNavLinkActive(item.href) ? "text-amber-400" : ""
                  }`}
                >
                  {item.label}
                </Link>
            ...
            <Link
              href="/contact"
              className="inline-flex items-center justify-center rounded-full bg-amber-400 px-6 py-2.5 text-sm font-bold text-slate-900 transition hover:bg-amber-300 shadow-lg shadow-amber-400/10"
            >
              Enquire Now
            </Link>
          </div>
          ...
          <div className="flex items-center gap-2 lg:hidden">
            <Link
              href="/contact"
              className="hidden min-[380px]:inline-flex items-center justify-center rounded-full bg-amber-400 px-3 py-2 text-xs font-bold text-slate-950 transition hover:bg-amber-300 sm:px-4 sm:text-sm"
            >
              Enquire
            </Link>
            <button
      ...
      <div className="lg:hidden">
        <div
          ...
          className={`fixed inset-0 z-[140] overflow-y-auto bg-slate-950/98 px-5 pb-8 pt-[calc(env(safe-area-inset-top)+1rem)] backdrop-blur-xl transition-all duration-300 sm:px-6 ${
            isOpen
              ...
              <nav className="mt-4 flex flex-col gap-2" aria-label="Mobile Primary">
                {navigation.map((item) => (
                  <Link
                    key={item.href}
                    href={item.href}
                    className={`flex items-center justify-between rounded-2xl px-4 py-3 text-base font-semibold transition-all duration-300 ${
